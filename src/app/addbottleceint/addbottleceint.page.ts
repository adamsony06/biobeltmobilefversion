  ...
  onRemove() {
    this.scan.scan().then(res=>{
        ...
        if(scanned){
          ...
          if(this.B1Desig.length == 0) { this.B1Ad = [];}
          if(this.B2Desig.length == 0) { this.B2Ad = [];}
        }else{
  ...
  removeB1 () {
    return new Promise(async (resolve, reject)=>{
      for (var i = 41119;i<=41159;i+=10){
        this.upc.client.setStringInHoldingRegister(i,"\0").then(res=>{
